    ...
    <div className="slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
      ...
      {logs.length > 0 && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {Object.entries(levelCounts).sort(([,a],[,b]) => b-a).map(([level, count]) => (
            <span key={level} style={{ fontSize: 'var(--font-sm)', fontFamily: 'monospace', padding: '3px 8px', borderRadius: 4, background: (LEVEL_COLORS[level] || '#64748b') + '20', color: LEVEL_COLORS[level] || '#64748b', border: '1px solid ' + (LEVEL_COLORS[level] || '#64748b') + '40' }}>
              {level}: {count}
            </span>
          ))}
        </div>
      ...
      {logs.length > 0 && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <input type="text" value={filterText} onChange={e => setFilterText(e.target.value)}
            ...
            style={{ flex: 1, minWidth: 200, background: 'var(--vespa-panel)', border: '1px solid var(--vespa-border)', borderRadius: 4, padding: '6px 10px', color: '#e2e8f0', fontFamily: 'monospace', fontSize: 'var(--font-base)', outline: 'none' }} />
          {LEVELS.map(l => (
            <button key={l} onClick={() => setFilterLevel(l)}
              style={{ fontSize: 'var(--font-xs)', color: filterLevel === l ? (LEVEL_COLORS[l] || '#e2e8f0') : '#64748b', background: 'none', border: '1px solid ' + (filterLevel === l ? (LEVEL_COLORS[l] || '#e2e8f0') : 'var(--vespa-border)'), borderRadius: 4, padding: '3px 8px', cursor: 'pointer', fontFamily: 'monospace' }}>
              {l}
            </button>
          ))}
          <span style={{ fontSize: 'var(--font-sm)', color: '#64748b', fontFamily: 'monospace' }}>{filtered.length}/{logs.length}</span>
        </div>
      )}
      ...
      {filtered.length > 0 && (
        <div style={{ background: 'var(--vespa-bg)', border: '1px solid var(--vespa-border)', borderRadius: 6, overflow: 'auto', flex: 1, minHeight: 300 }}>
          <div style={{ display: 'table', width: '100%', fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--font-sm)' }}>
            {filtered.map((line, i) => (
