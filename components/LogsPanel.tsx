    ...
    <div className="slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
      <div style={{ background: 'var(--vespa-panel)', border: '1px solid var(--vespa-border)', borderRadius: 6, padding: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
          <span style={{ color: '#818cf8', fontFamily: 'monospace', fontSize: 12, fontWeight: 600, letterSpacing: '0.08em' }}>LOG SOURCE</span>
          <div style={{ display: 'flex', gap: 4 }}>
            {(['api', 'paste'] as const).map(s => (
              <button key={s} onClick={() => setSource(s)}
                style={{ fontSize: 11, color: source === s ? 'var(--vespa-accent)' : '#64748b', background: 'none', border: '1px solid ' + (source === s ? 'var(--vespa-accent)' : 'var(--vespa-border)'), borderRadius: 4, padding: '3px 10px', cursor: 'pointer', fontFamily: 'monospace' }}>
        ...
        {source === 'api' ? (
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div>
              <label style={{ fontSize: 10, color: '#64748b', display: 'block', fontFamily: 'monospace', marginBottom: 3 }}>Last N seconds</label>
              <input type="number" value={seconds} onChange={e => setSeconds(e.target.value)}
                style={{ background: 'var(--vespa-bg)', border: '1px solid var(--vespa-border)', borderRadius: 4, padding: '5px 8px', color: '#e2e8f0', fontFamily: 'monospace', fontSize: 12, outline: 'none', width: 100 }} />
            </div>
            <div>
              <label style={{ fontSize: 10, color: '#64748b', display: 'block', fontFamily: 'monospace', marginBottom: 3 }}>Component filter</label>
              <input type="text" value={component} onChange={e => setComponent(e.target.value)}
                placeholder="e.g. qrserver"
                style={{ background: 'var(--vespa-bg)', border: '1px solid var(--vespa-border)', borderRadius: 4, padding: '5px 8px', color: '#e2e8f0', fontFamily: 'monospace', fontSize: 12, outline: 'none', width: 160 }} />
            </div>
            <button onClick={fetchLogs} disabled={loading}
              style={{ background: loading ? '#1a2a35' : 'var(--vespa-accent)', color: loading ? '#64748b' : '#0c0e11', border: 'none', borderRadius: 6, padding: '7px 18px', fontWeight: 600, fontSize: 13, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'IBM Plex Sans' }}>
              {loading ? '⟳' : '⬇ Fetch'}
            </button>
          </div>
        ) : (
          <div>
            ...
            />
            <button onClick={fetchLogs} style={{ marginTop: 8, background: 'var(--vespa-accent)', color: '#0c0e11', border: 'none', borderRadius: 6, padding: '7px 18px', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'IBM Plex Sans' }}>
              Parse Logs
            </button>
          </div>
        )}
