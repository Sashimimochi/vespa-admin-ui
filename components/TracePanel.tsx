  ...
  return (
    <div className="slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ background: 'var(--vespa-panel)', border: '1px solid var(--vespa-border)', borderRadius: 6, padding: 16 }}>
        <div style={{ color: '#818cf8', fontFamily: 'monospace', fontSize: 'var(--font-base)', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 10 }}>QUERY ANALYZER</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 8 }}>
          <div>
            <label style={{ display: 'block', fontSize: 'var(--font-sm)', color: '#64748b', marginBottom: 3, fontFamily: 'monospace' }}>YQL</label>
            <textarea
          ...
          {needsQuery && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#2a1a00', border: '1px solid #f59e0b', borderRadius: 4, padding: '8px 12px' }}>
              ...
              <span style={{ fontSize: 'var(--font-base)', color: '#fbbf24', fontFamily: 'monospace', lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px', gap: 8 }}>
            <div>
              <label style={{ display: 'block', fontSize: 'var(--font-sm)', color: needsQuery ? '#f59e0b' : '#64748b', marginBottom: 3, fontFamily: 'monospace' }}>
                query{needsQuery ? ' ⚠ 必須: userQuery() の展開テキスト' : ' (userQuery() の展開テキスト)'}
              </label>
              <input type="text" value={query} onChange={e => setQuery(e.target.value)}
                placeholder="例: vespa search"
                style={{ width: '100%', background: 'var(--vespa-bg)', border: `1px solid ${needsQuery ? '#f59e0b' : 'var(--vespa-border)'}`, borderRadius: 4, padding: '5px 8px', color: '#e2e8f0', fontFamily: 'monospace', fontSize: 'var(--font-base)', outline: 'none' }}
              />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 'var(--font-sm)', color: '#64748b', marginBottom: 3, fontFamily: 'monospace' }}>trace.level (1-9)</label>
              <select value={traceLevel} onChange={e => setTraceLevel(e.target.value)}
                style={{ width: '100%', background: 'var(--vespa-bg)', border: '1px solid var(--vespa-border)', borderRadius: 4, padding: '5px 8px', color: '#e2e8f0', fontFamily: 'monospace', fontSize: 'var(--font-base)', outline: 'none' }}>
                {[1,2,3,4,5,6,7,8,9].map(n => <option key={n} value={n}>{n}</option>)}
              </select>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 'var(--font-sm)', color: '#64748b', marginBottom: 3, fontFamily: 'monospace' }}>language</label>
        ...
        <button onClick={runTrace} disabled={loading || needsQuery}
          style={{ marginTop: 12, background: (loading || needsQuery) ? '#1a2a35' : 'var(--vespa-accent)', color: (loading || needsQuery) ? '#64748b' : '#0c0e11', border: 'none', borderRadius: 6, padding: '8px 20px', fontWeight: 600, fontSize: 'var(--font-md)', cursor: (loading || needsQuery) ? 'not-allowed' : 'pointer', fontFamily: 'IBM Plex Sans' }}>
          {loading ? '⟳ Analyzing...' : needsQuery ? '⚠ query を入力してください' : '🔬 Analyze Query'}
        </button>
      </div>

      {/* Hint about tokens debug summary */}
      <div style={{ background: '#1a1f29', border: '1px solid #252b38', borderRadius: 6, padding: 12, fontSize: 'var(--font-sm)', color: '#64748b', lineHeight: 1.7 }}>
...
{`document-summary debug-summary {
  ...
  summary myfield_tokens { source: myfield tokens }
      ...
      {error && (
        <div style={{ background: '#1a0f0f', border: '1px solid #ef4444', borderRadius: 6, padding: 12, color: '#ef4444', fontFamily: 'monospace', fontSize: 'var(--font-base)' }}>
          ...
          {yql.includes('userQuery()') && !query && (
            <div style={{ marginTop: 8, color: '#fbbf24', fontSize: 'var(--font-sm)' }}>
      ...
      {tokenInfo.length > 0 && (
        <div style={{ background: 'var(--vespa-panel)', border: '1px solid var(--vespa-border)', borderRadius: 6, padding: 16 }}>
          <div style={{ color: '#00b4d8', fontFamily: 'monospace', fontSize: 'var(--font-base)', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 10 }}>INDEXED TOKENS (from debug summary)</div>
          {tokenInfo.map(ti => (
            <div key={ti.field} style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 'var(--font-sm)', color: '#818cf8', fontFamily: 'monospace', marginBottom: 4 }}>{ti.field}</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {ti.tokens.map((tok, i) => (
                  <span key={i} style={{ background: '#0d1b24', border: '1px solid #00b4d8', borderRadius: 4, padding: '2px 8px', fontSize: 'var(--font-sm)', fontFamily: 'monospace', color: '#00b4d8' }}>
                    {tok}
                  </span>
                ))}
              </div>
      ...
      {messages.length > 0 && (
        <div style={{ background: 'var(--vespa-panel)', border: '1px solid var(--vespa-border)', borderRadius: 6, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ color: '#818cf8', fontFamily: 'monospace', fontSize: 'var(--font-base)', fontWeight: 600, letterSpacing: '0.08em' }}>
              TRACE LOG ({messages.length} messages)
            </span>
            <div style={{ display: 'flex', gap: 6 }}>
              {types.map(t => (
                <button key={t} onClick={() => setFilterType(t)}
                  style={{ fontSize: 'var(--font-xs)', color: filterType === t ? (TYPE_COLORS[t] || '#e2e8f0') : '#64748b', background: 'none', border: '1px solid ' + (filterType === t ? (TYPE_COLORS[t] || '#e2e8f0') : 'var(--vespa-border)'), borderRadius: 4, padding: '2px 6px', cursor: 'pointer', fontFamily: 'monospace' }}>
                  {t}
                </button>
              ))}
              <button onClick={() => setShowRaw(!showRaw)}
                style={{ fontSize: 'var(--font-xs)', color: '#64748b', background: 'none', border: '1px solid var(--vespa-border)', borderRadius: 4, padding: '2px 6px', cursor: 'pointer', fontFamily: 'monospace' }}>
          ...
          {showRaw ? (
            <pre style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'var(--font-sm)', color: '#a8d8ea', overflow: 'auto', maxHeight: 400, margin: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1, maxHeight: 500, overflow: 'auto' }}>
              {filtered.map((m, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 'var(--font-xs)', color: '#3a4252', fontFamily: 'monospace', flexShrink: 0, marginTop: 1 }}>{String(i).padStart(3, '0')}</span>
                  <span style={{ fontSize: 'var(--font-xs)', padding: '1px 5px', borderRadius: 3, background: TYPE_COLORS[m.type] + '20', color: TYPE_COLORS[m.type], fontFamily: 'monospace', flexShrink: 0, alignSelf: 'flex-start' }}>
                    {m.type}
                  </span>
                  <span style={{ fontSize: 'var(--font-base)', color: '#cbd5e1', fontFamily: 'JetBrains Mono, monospace', lineHeight: 1.5, wordBreak: 'break-all' }}>
                    {m.message}
                  </span>
                </div>
              ))}
